...
function KoiEditor(options){
    ...
    _self._init = function() {
        ...
        if(settings['saveTo'].length <= 0){
            alert('KoiEditor. Please define the saveTo option.');
            initErrors ++;
        }
        if(settings['saveId'].length <= 0){
            alert('KoiEditor. Please define the saveId option.');
            initErrors ++;
        }
    ...
    _self.keSave = function keSave() {
        ...
        $.post( settings['saveTo'], { id: settings['saveId'], content: $('#' + keEditableContentDiv).html() } );
    }
